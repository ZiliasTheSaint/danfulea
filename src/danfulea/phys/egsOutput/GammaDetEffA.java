 ...
 * and detector efficiency (peak if photons and global if photons, electrons and positrons). <br>
 * Start as minor modification of GammaDetEff regarding variables used in handling geometry. 
 * End up all future improvements were made on this class. Bottom line: use this class instead of GammaDetEff.
 * @author Dan Fulea, 05 DEC. 2005
 */

public class GammaDetEffA implements EgsQuestion {
	...
	public GammaDetEffA() {
		...
		String fs = cal.get(Calendar.YEAR) + "_" + cal.get(Calendar.MONTH)
				+ "_" + cal.get(Calendar.DAY_OF_MONTH) + "_"
				+ cal.get(Calendar.HOUR) + "_" + cal.get(Calendar.MINUTE) + "_"
	...
	private void destroyArrays() {
		AMASS = new double[0][0];
		SCDOSE = new double[0][0][0];
		SCDOSE2 = new double[0][0][0];
		SCKERMA = new double[0][0][0];
		SCKERMA2 = new double[0][0][0];
		SCDOSEtoKERMA2 = new double[0][0][0];
		SCDOSE_LAST = new int[0][0][0];// ($MAXZREG,$MAXRADII,$MAXIT),
		SCKERMA_LAST = new int[0][0][0];
		SCKERMA_LASTOLD = new int[0][0][0];
		SCDOSE_TMP = new double[0][0][0];
		SCKERMA_TMP = new double[0][0][0];
		SCKERMA_TMPOLD = new double[0][0][0];
		// ($MAXZREG,$MAXRADII,$MAXIT)
		IPHR = new int[0];
		IDSTBL = new int[0][0];// ($MXREG,2);
	...
	private static void getCylinderRandom() {
		...
		// -----------------------------------
		// double ux=u;double uy=v;double uz=w;
		// strictly in source volume
		// double l1=0.0;
		// double
		// zer=(ux*x0+uy*y0)*(ux*x0+uy*y0)+(ux*ux+uy*uy)*(asource*asource-x0*x0-y0*y0);
		/*
		 * if (zer>0.) { double
	...
	private void getMCylInf(double z1) {
		...
		// double l1=0.0;
		// double
		// zer=(ux*x0+uy*y0)*(ux*x0+uy*y0)+(ux*ux+uy*uy)*(bsource*bsource-x0*x0-y0*y0);
		/*
		 * if (zer>0.) { double
		 * s1=(-(ux*x0+uy*y0)-Math.sqrt((ux*x0+uy*y0)*(ux*x0+uy*y0)+
	...
	private void getMMidInf(double z1) {
		...
		// (ux*ux+uy*uy)*(EGS4Geom.RCYL[EGS4Geom.NR]*EGS4Geom.RCYL[EGS4Geom.NR]-x0*x0-y0*y0);
		/*
