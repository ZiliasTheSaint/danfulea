...
/**
 * Class for visualize a 3D HalfClosedEllipsoid. It is based on old java3D tutorials freely available on the web (at that time).
 * 
 * @author Dan Fulea, 12 MAY. 2005
 *
 */

public class HalfClosedEllipsoid extends Shape3D {
	...
	public HalfClosedEllipsoid(int strips, int stripLength, Color4f c1,
	...
	private double radiusx(int strip, int segment) {
		...
		// double radius=2*a*Math.abs((strip + 1 -
	...
	private double radiusz(int strip, int segment) {
		...
		if (strip < (double) strips / 2)
			...
			radius = 2 * c * Math.abs((strip + 1 - segment - (double) strips))
					/ (double) strips;
		// double radius=2*c*Math.abs((strip + 1 -
		// segment-(double)strips/2))/(double)strips;
		return radius;
		// -----------------------------------------
	...
	public double y(int strip, int index, int segment) {
		...
		if (strip >= (double) strips / 2)
			...
			yy = -b
					* Math.sqrt(Math.abs(1.0 - (x * x / (a * a) + z * z
							/ (c * c))));
		return yy;
	}
