...
/**
 * Class for visualize a 3D HalfEllipsoid. It is based on old java3D tutorials freely available on the web (at that time).
 * 
 * @author Dan Fulea, 12 MAY. 2005
 *
 */

public class HalfEllipsoid extends Shape3D {
	...
	public HalfEllipsoid(int strips, int stripLength, Color4f c1, Color4f c2,
	...
	private double radiusx(int strip, int segment) {
		...
		// ----------------------------------------------------
		return a * (strip + 1 - segment) / (double) strips;
		// ----------------------------------------------------
	...
	private double radiusz(int strip, int segment) {
		...
		// return (strip + 1 - segment)/(double)strips;
		// ----------------------------------------------------
		return c * (strip + 1 - segment) / (double) strips;
		// ----------------------------------------------------
	...
	private double y(int strip, int index, int segment) {
		...
		double z = z(strip, index, segment);
		double yy = -b
				* Math.sqrt(Math.abs(1.0 - (x * x / (a * a) + z * z / (c * c))));
		return yy;
	}
