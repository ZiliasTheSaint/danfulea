...
/**
 * Class for visualize a 3D Cone. It is based on old java3D tutorials freely available on the web (at that time).
 * 
...
public class Cone extends Shape3D{
	// stripLength is the number of points plotted in each segment of the strip.
		...
		public Cone(int strips, int stripLength, Color4f c1, Color4f c2// )
				, Appearance appearance, double rr, double h) {
			this.rr = rr;// radius
			this.h = h;// height
			// -----------THIS PART IS COMMON--------------------------------------
		...
		private double radius(int strip, int segment) {
			...
			return rr * (strip + 1 - segment) / ((double) strips);
