 ...
 */
public class Tutor7 implements EgsQuestion {
	...
	private int $MXEBIN = 200;
	private int icase = 0;
	private int ipulse = 0;
	...
	private void init() {
		...
		sc_pulse_height = new double[$MXEBIN][EGS4.$MXREG];// ($MXEBIN,$MXREG)
		...
		if (ipulse == 1) {// "Output pulse height distributions "
			...
			EGS4.seqStr = " ----------------Pulse Height Distributions--------------";
			if (EGS4.iprint > 0)
				printSequence(EGS4.seqStr);
			...
			for (I = 1; I <= nzb + 1; I++) {
				...
				for (J = 2; J <= $MXEBIN; J++) {
					...
					double dd = (J - 0.5) * de_pulse;

					EGS4.seqStr = EGS4.format(dd, 9, true)
							+ EGS4.format(aux, 15) + EGS4.format(aux2, 15);
					if (EGS4.iprint > 0)
						printSequence(EGS4.seqStr);
					// write(1,'(f9.4,2g15.6)')
					// (float(j)-0.5)*de_pulse,aux,aux2;
				}
				// write(1,*);
				EGS4.seqStr = " \n";
				if (EGS4.iprint > 0)
		...
		if (iarg < 5) {// "energy is being deposited"
			...
			if (icase == sc_last[irl - 1]) {// "still the same shower that deposited energy"
				...
				if (ipulse == 1) {
					...
					jp = dbl.intValue();
					if ((jp > 0) && (jp <= $MXEBIN)) {
						// sc_pulse_height(jp,irl) = sc_pulse_height(jp,irl) +
						// 1;
	...
	private void inputs() {
		...
		if (iterse == 0)// //"User wants to input groups of planes "
		{
			...
			// %%%%%%%%%%%%%INPUT%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
			num_slab[0] = 30;// 1;//'NUMBER OF SLABS'; "The code word"
			zfront[0] = 0.0;// value(num_zfront,1);==='Z OF FRONT FACE'
							// "The code word"
			deltaz[0] = 0.01;// 1.0;////slab thickness= 0.01;value(num_deltaz,i)
			// %%%%%%%%%%%%%END INPUT%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

			zbound[0] = zfront[0];
			nzb = 1;
			for (I = 1; I <= n_groups; I++)// ,nvalue(num_nslab) [
			{
				for (J = 1; J <= num_slab[I - 1]; J++)// ,value(num_nslab,i) [
				{
					nzb = nzb + 1;
					if (nzb > EGS4.$MXREG) {
						EGS4.STOPPROGRAM = true;
		...
		// GEOMETRY##############################################
		// #####################" Media input "##########################################
		EGS4.MEDIA[0] = "TA_fortran";// "GE_fortran";//"TA_fortran";
		...
		EGS4.DUNIT = 1; // "i.e. we work in cm"
		...
		// 'PULSE HEIGHT DISTRIBUTION'=Yes=>1 No=>0
		// ipulse = 1;//no for Ta
		...
		irejct = 0;// off//value(ival,1);
		EGS4Macro.irejct = irejct;
		// Esave= 1
		if (irejct == 1) {
			esave = 1.0;// value(ival,1);
			EGS4Macro.esave = esave;
		}
		// " Source inputs "
		IQIN = -1;// 0;//'INCIDENT CHARGE'
		EIN = 10.0;// 0.2;//'INCIDENT KINETIC ENERGY'
		if (IQIN != 0)
			EIN = EIN + EGS4.RM;
		NCASE = 1000;// 100000;//5000000;//'NUMBER OF HISTORIES'
		...
		XIN = 0.0;
		YIN = 0.0;
		ZIN = zbound[0];// //zbound(1);

		if (ipulse == 1) {
			// de_pulse = 1.05*value(num_energy,1)/$MXEBIN;
			de_pulse = 1.05 * EIN / $MXEBIN;
		}
