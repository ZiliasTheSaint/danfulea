...
/**
 * Class for visualize a 3D RightClosedCone. It is based on old java3D tutorials freely available on the web (at that time).
 * 
 * @author Dan Fulea, 12 MAY. 2005
 *
 */

public class RightClosedCone extends Shape3D {
	...
	public RightClosedCone(int strips, int stripLength, Color4f c1, Color4f c2,
	...
	private double radius(int strip, int segment) {
		...
		if (strip < (double) strips / 2)
			d = at * (strip + 1 - segment) / ((double) strips);
		else
			d = at * Math.abs((strip + 1 - segment - (double) strips))
					/ (double) strips;
	...
	private double x(int strip, int index, int segment) {
		double d = -radius(strip, segment) + radius(strip, segment)
	...
	private double z(int strip, int index, int segment) {
		double zz = 0.0;
		if (strip < (double) strips / 2)
			zz = -cl + 2 * cl * (strip + 1 - segment) / ((double) strips);
		else
			zz = 0.0;
		return zz;
	}
