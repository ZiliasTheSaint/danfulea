...
/**
 * Class for visualize a 3D RightClosedEllipsoid. It is based on old java3D tutorials freely available on the web (at that time).
 * 
 * @author Dan Fulea, 12 MAY. 2005
 *
 */

public class RightClosedEllipsoid extends Shape3D {
	...
	public RightClosedEllipsoid(int strips, int stripLength, Color4f c1,
			Color4f c2, Appearance appearance// )
			, double a, double b, double c) {
		// strips should be multiple of 5: eg 15!!!
		this.a = a;
		this.b = b;
		this.c = c;
		// -----------THIS PART IS COMMON--------------------------------------
	...
	private double radiusx(int strip, int segment) {
		...
		// ----------------------------------------------------------------------
		double radius = 0.0;
		if (strip >= (double) strips / 3 && strip < 2 * (double) strips / 3)
			radius = a;
		else if (strip < (double) strips / 3)
			// radius=3*a*Math.abs((strip + 1 -
			// segment-(double)strips/3))/(double)strips;
			radius = 3 * a * Math.abs((strip + 1 - segment)) / (double) strips;
		else
			// radius=3*a*Math.abs((strip + 1 -
			// segment-2*(double)strips/3))/(double)strips;
			radius = 3 * a * Math.abs((strip + 1 - segment - (double) strips))
					/ (double) strips;
		return radius;
		// ----------------------------------------------------------------------
	...
	private double radiusz(int strip, int segment) {
		...
		if (strip >= (double) strips / 3 && strip < 2 * (double) strips / 3)
			...
			// radius=3*c*Math.abs((strip + 1 -
			// segment-(double)strips/3))/(double)strips;
			radius = 3 * c * Math.abs((strip + 1 - segment)) / (double) strips;
		else
			// radius=3*c*Math.abs((strip + 1 -
			// segment-2*(double)strips/3))/(double)strips;
			radius = 3 * c * Math.abs((strip + 1 - segment - (double) strips))
	...
	private double y(int strip, int index, int segment) {
		double yy = 0.0;
		if (strip >= (double) strips / 3 && strip < 2 * (double) strips / 3)
			yy = 3 * b * (strip + 1 - segment - (double) strips / 3)
					/ (double) strips;
		else if (strip < (double) strips / 3)
			yy = 0.0;
		else
